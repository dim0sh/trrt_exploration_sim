...
#define MURL_RECTANGLE_FROM_MU(r) ((Rectangle){(float)(r.x), (float)(r.y), (float)(r.w), (float)(r.h)})

// Create a raylib Vector2 from a microui mu_Vec2.
#define MURL_VECTOR2_FROM_MU(v) ((Vector2){(float)(v.x), (float)(v.y)})

static inline int murl_text_width(mu_Font font, const char *str, int len) {
  (void)len;
...
static inline int murl_text_height(mu_Font font) {
  Font rlfont = MURL_FONT_FROM_MU(font);
  return rlfont.baseSize;
}

// Set the text height/width callbacks and the font.
static inline void murl_setup_font_ex(mu_Context *ctx, const Font *font) {
...
static inline void murl_handle_mouse_scroll(mu_Context *ctx) {
...
static inline void murl_handle_mouse_buttons_input_ex(mu_Context *ctx, int x, int y) {
...
static inline void murl_handle_keyboard_input(mu_Context *ctx) {
...
static inline void murl_handle_text_input(mu_Context *ctx) {
  char buffer[512];
...
static inline void murl_handle_input(mu_Context *ctx) {
...
static inline void murl_render(mu_Context *ctx) {
  mu_Command *cmd = NULL;
  while (mu_next_command(ctx, &cmd)) {
    ...
    case MU_COMMAND_TEXT: {
      ...
      float font_size = ctx->text_height(&font);
